...
const adminSignInOtp = async (req, res) => {
    try {
        ...
        if (!adminExists) {
            return res.status(404).json({
                success: false,
                message: "Invalid University Mail ID or Password!"
            });
        }

        bcrypt.compare(password, adminExists.password, async (err, result) => {
            if (err) {
            ...
            if (result) {
                try {
                    ...
                    return res.status(500).json({
                        success: false,
                        message: "Failed to generate/send OTP. Try again later.",
                        error: err.message
                    });
                }
            } else {
        ...
        console.log("Sign In OTP Generation Error : ", err.message);
        return res.status(500).json({
            success: false,
            error: err.message
        });
